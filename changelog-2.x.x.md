...
- Structure Config now includes 'Templates' and 'Template Pools' (aka 'Pools'). Both are required for structures using the 'jigsaw system'
...
- Press the arrow which is pointing down to extend bar on the Top to scale images and open 'Admin Mode' (aka 'Structure Config UI')
...
- Removes all blocks/entities in the list from the structures at placement
-> Can for example remove all dragon heads from the harbour
