...
@OnlyIn(Dist.CLIENT)
public abstract class IconList<E extends IconList.AbstractListEntry<E>> extends FocusableGui implements IRenderable {
	...
	@SuppressWarnings("deprecation")
	protected void renderList(MatrixStack matrixStack, int scrollAmount, int mouseX, int mouseY, float p_238478_6_) {
		int i = this.getItemCount();
		Tessellator tessellator = Tessellator.getInstance();
		BufferBuilder bufferbuilder = tessellator.getBuilder();
		...
		for (int j = 0; j < i; ++j) {
			...
			if (this.isSelectedItem(j)) {
				...
				int y4 = (int) (this.getEntryTop(j) + this.itemSize + 2);
				...
				bufferbuilder.vertex((double) x3, (double) y4, 0.0D).endVertex();
				bufferbuilder.vertex((double) x4, (double) y4, 0.0D).endVertex();
				bufferbuilder.vertex((double) x4, (double) y3, 0.0D).endVertex();
				bufferbuilder.vertex((double) x3, (double) y3, 0.0D).endVertex();
				tessellator.end();
				RenderSystem.color4f(0.0F, 0.0F, 0.0F, 1.0F);
				bufferbuilder.begin(7, DefaultVertexFormats.POSITION);
				bufferbuilder.vertex((double) (x3 - 1), (double) (y4 + 1), 0.0D).endVertex();
				bufferbuilder.vertex((double) (x4 + 1), (double) (y4 + 1), 0.0D).endVertex();
				bufferbuilder.vertex((double) (x4 + 1), (double) (y3 - 1), 0.0D).endVertex();
				bufferbuilder.vertex((double) (x3 - 1), (double) (y3 - 1), 0.0D).endVertex();
				tessellator.end();
				RenderSystem.enableTexture();
			}
	...
	public abstract static class AbstractListEntry<E extends IconList.AbstractListEntry<E>>
		...
		public abstract void render(MatrixStack p_230432_1_, int p_230432_2_, int p_230432_3_, int p_230432_4_,
