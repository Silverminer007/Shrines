...
public abstract class PlacementCalculator {
   public static final Codec<PlacementCalculator> CODEC = ResourceLocation.CODEC.flatXmap(
         resourceLocation ->
               Optional.ofNullable(PlacementCalculatorTypeRegistry.REGISTRY_SUPPLIER.get().getValue(resourceLocation))
                     .map(DataResult::success).orElse(DataResult.error(resourceLocation + " is no valid SpawnCriteria Type")),
         spawnCriteria -> Optional.ofNullable(PlacementCalculatorTypeRegistry.REGISTRY_SUPPLIER.get().getKey(spawnCriteria))
