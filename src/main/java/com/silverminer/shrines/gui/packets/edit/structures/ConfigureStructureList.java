...
import com.silverminer.shrines.gui.misc.SingleSelectListScreen;
...
public class ConfigureStructureList extends ExtendedList<ConfigureStructureList.Entry<?>> {
   ...
   public void refreshList() {
      ...
      this.addEntry(new SingleSelectEntry<>(this.structure.getHeightmapType(), () -> Arrays.asList(Heightmap.Type.values()), this.structure::setHeightmapType, Enum::toString, ConfigOptions.heightmap_type));
   }
   ...
   public abstract class Entry<T>
      ...
      public void render(MatrixStack ms, int index, int top, int left, int width, int height, int mouseX, int mouseY,
         ...
         String description = new TranslationTextComponent("options.shrines." + this.getOptionString()).getString();
         ...
         if ((mouseX >= left) && (mouseX < (left + descriptionWidth))
            ...
            List<ITextComponent> comment = Arrays.stream(this.getComments())
      ...
      protected String[] getComments() {
         return this.option.getComments();
      }
   ...
   @OnlyIn(Dist.CLIENT)
   public class SingleSelectEntry<T> extends Entry<T> {
      protected final Button button;
      protected final Function<T, String> toString;
      protected final String optionId;

      public SingleSelectEntry(@Nullable T value, Supplier<List<T>> possibleValues, Consumer<T> valueUpdater, Function<T, String> toString, String optionId) {
         super(null, () -> value, valueUpdater);
         this.button = new Button(0, 0, 70, 20, new TranslationTextComponent("gui.shrines.configure"),
               (button) -> this.minecraft.setScreen(
                     new SingleSelectListScreen<>(ConfigureStructureList.this.screen,
                           new TranslationTextComponent("gui.shrines.configure_element", optionId),
                           value, possibleValues, valueUpdater, toString)));
         this.children.add(this.button);
