...
public class ConfigureStructureList extends ExtendedList<ConfigureStructureList.Entry<?>> {
	...
	public void refreshList() {
		this.clearEntries();
		this.addEntry(
				new StringEntry(this.structure.name, this.structure::getName, this.structure::setName, 256, text -> {
					return true;
				}));
		this.addEntry(new StringEntry(this.structure.key, this.structure::getKey, this.structure::setKey, 256,
				text -> this.structure.successful));
		this.addEntry(
	...
	public abstract class Entry<T extends Object>
		...
		public void render(MatrixStack ms, int index, int top, int left, int width, int height, int mouseX, int mouseY,
			...
			if ((mouseX >= descriptionLeft) && (mouseX < (descriptionLeft + descriptionWidth))
				...
				List<ITextComponent> comment = Arrays.asList(this.option.getComments()).stream()
						.map(s -> new TranslationTextComponent(s)).collect(Collectors.toList());
	...
	public class StringEntry extends Entry<String> {
		...
		public StringEntry(ConfigOption<String> option, Supplier<String> value, Consumer<String> saver, int maxLenght,
				Function<String, Boolean> validator) {
			super(option, value, saver);
			this.textField = new TextFieldWidget(this.minecraft.font, 0, 0, 200, 20, StringTextComponent.EMPTY);
			this.textField.setMaxLength(maxLenght);
	...
	public class StringListEntry extends Entry<List<String>> {
		...
		public StringListEntry(ConfigOption<List<String>> option, Supplier<List<String>> value,
			...
			super(option, value, saver);
			this.button = new Button(0, 0, 70, 20, new TranslationTextComponent("Configure"), (button) -> {
				this.minecraft.setScreen(new StringListOptionsScreen(ConfigureStructureList.this.screen, possibleValues,
						Lists.newArrayList(this.getter.get()), this.option.getOption(), saver));
			});// TRANSLATION
			this.children.add(this.button);
		}
	...
	public class BiomeListsEntry extends Entry<List<String>> {
		...
		public BiomeListsEntry(ConfigOption<List<String>> biomesOption, Supplier<List<String>> biomes,
			...
			this.button = new Button(0, 0, 70, 20, new TranslationTextComponent("Configure"), (button) -> {
				this.minecraft.setScreen(new BiomeGenerationSettingsScreen(screen, this.getter.get(),
						this.categoriesGetter.get(), this.option.getOption(), this.setter, this.categoriesSetter));
			});// TRANSLATION
			this.children.add(this.button);
		}
