...
public class NewGeneralSettingsList extends ObjectSelectionList<NewGeneralSettingsList.Entry<?>> {
   ...
   public void refreshList() {
      this.clearEntries();
      this.addEntry(new DoubleEntry("distance_factor",
            Config.SETTINGS.DISTANCE_FACTOR::get, Config.SETTINGS.DISTANCE_FACTOR::set));
      this.addEntry(new DoubleEntry("seperation_factor",
            Config.SETTINGS.SEPARATION_FACTOR::get, Config.SETTINGS.SEPARATION_FACTOR::set));
      this.addEntry(new IntegerEntry("structure_min_distance",
            Config.SETTINGS.STRUCTURE_MIN_DISTANCE::get, Config.SETTINGS.STRUCTURE_MIN_DISTANCE::set, false, false));
      this.addEntry(new StringListEntry("blacklist",
            () -> Config.SETTINGS.BLACKLISTED_BIOMES.get().stream().map(String::toString).collect(Collectors.toList()),
            Config.SETTINGS.BLACKLISTED_BIOMES::set,
            ForgeRegistries.BIOMES.getValues().stream().map(o -> o.getRegistryName() != null ? o.getRegistryName().toString() : null).filter(Objects::nonNull).collect(Collectors.toList())));
      this.addEntry(new BooleanEntry("advanced_logging",
            Config.SETTINGS.ADVANCED_LOGGING::get, Config.SETTINGS.ADVANCED_LOGGING::set));
      this.addEntry(new StringListEntry("banned_blocks",
            () -> Config.SETTINGS.BANNED_BLOCKS.get().stream().map(String::toString).collect(Collectors.toList()),
            Config.SETTINGS.BANNED_BLOCKS::set,
            ForgeRegistries.BLOCKS.getValues().stream().map(o -> o.getRegistryName() != null ? o.getRegistryName().toString() : null).filter(Objects::nonNull).collect(Collectors.toList())));
      this.addEntry(new StringListEntry("banned_entities",
            () -> Config.SETTINGS.BANNED_ENTITIES.get().stream().map(String::toString).collect(Collectors.toList()),
            Config.SETTINGS.BANNED_ENTITIES::set,
            ForgeRegistries.ENTITIES.getValues().stream().map(o -> o.getRegistryName() != null ? o.getRegistryName().toString() : null).filter(Objects::nonNull).collect(Collectors.toList())));
   }
