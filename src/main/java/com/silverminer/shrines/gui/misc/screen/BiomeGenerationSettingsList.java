    ...
    public void refreshList(Supplier<String> search) {
        ...
        for (Biome.BiomeCategory opt : possibleCategories) {
            if (opt.getName().toLowerCase(Locale.ROOT).contains(s)) {
                ...
                if (selected) {
                    for (Biome opt1 : possibleBiomes.stream().filter(biome -> Biome.getBiomeCategory(Holder.direct(biome)).equals(opt)).toList()) {
        ...
        public void updateList(boolean active) {
            if (active) {
                ...
                for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
                    if (Biome.getBiomeCategory(Holder.direct(biome)).toString().equals(this.opt)) {
