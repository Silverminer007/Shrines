...
public class CustomStructureInit {
	...
	public static void registerStructures(Register<Structure<?>> event) {
		...
		for (CustomStructureData csd : Utils.customsStructs) {
			...
			cS.setRegistryName(ShrinesMod.MODID, name);
			if (!Structure.STRUCTURES_REGISTRY.containsValue(cS)) {
				Structure.STRUCTURES_REGISTRY.putIfAbsent(new ResourceLocation(ShrinesMod.MODID, name).toString(), cS);
			}
			...
			USERS_STRUCTURES.add(RegistryObject.of(new ResourceLocation(ShrinesMod.MODID, name), ForgeRegistries.STRUCTURE_FEATURES));
		}
