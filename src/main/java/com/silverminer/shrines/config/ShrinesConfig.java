...
public class ShrinesConfig {
   ...
   private static void setupConfig(ForgeConfigSpec.@NotNull Builder builder) {
      removedStructures = builder.comment("You should only need to use this option in very edge case scenarios. This option transforms all structures in this list to shrines:deleted_structures when Minecraft updates your world from 1.18.2 or below to 1.18.2 or above").define("removed_structures", new ArrayList<>());
      ...
      min_structure_distance = builder.comment("An additional spawn requirement for shrines structures. This only affects structures which use the shrines:min_structure_distance spawn criteria and they're is set to a negative value. This is the case for all shrines default structures in default configuration. Set this to a negative value to disable min structure distance entirely. ").defineInRange("min_structure_distance", 10, 0, 100);
   }
}