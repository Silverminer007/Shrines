...
public abstract class SpawnCriteria {
   public static final Codec<SpawnCriteria> CODEC = ResourceLocation.CODEC.flatXmap(
         resourceLocation ->
               Optional.ofNullable(SpawnCriteriaTypeRegistry.REGISTRY_SUPPLIER.get().getValue(resourceLocation))
                     .map(DataResult::success).orElse(DataResult.error(resourceLocation + " is no valid SpawnCriteria Type")),
         spawnCriteria -> Optional.ofNullable(SpawnCriteriaTypeRegistry.REGISTRY_SUPPLIER.get().getKey(spawnCriteria))
